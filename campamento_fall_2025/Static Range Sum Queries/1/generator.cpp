...
int main() {
    ...
    string basePath = "D:\\Projects\\CPC-UAEH\\campamento_fall_2025\\Static Range Sum Queries\\1\\cases\\case";
    ...
    for (int id = 1; id <= numCases; id++) {
        ...
        // Calculamos la respuesta correcta (suma en [L, R])
        long long suma = 0;
        for (int i = L; i <= R; i++) {
            suma += (long long)a[i];
        }
        ...
        // Escribimos la respuesta esperada
        salidaOut << suma << "\n";
        salidaOut.close();
    }
